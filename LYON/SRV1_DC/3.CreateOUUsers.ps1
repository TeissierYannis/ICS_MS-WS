...
New-ADOrganizationalUnit -Name "Site" -ProtectedFromAccidentalDeletion:$false
...
foreach($OU in $OUS)
{
    ...
    If ($splitted.Length -eq 1) {
        ...
        New-ADOrganizationalUnit -Name $splitted.Get(0) -Path $path -ProtectedFromAccidentalDeletion:$false
    } else {
        $path = "OU=" + $splitted.Get(0) + ",OU=Site,DC=ESN,DC=dom"
        New-ADOrganizationalUnit -Name $splitted.Get(1) -Path $path -ProtectedFromAccidentalDeletion:$false
